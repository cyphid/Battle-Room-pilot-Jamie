...
def move(game_state: typing.Dict = None) -> typing.Dict:
  ...
  for snake in game_state.board.snakes: #avoid head to head collisions
    ...
    if snake.head.x == game_state.you.body[0].x -1 and snake.head.y == game_state.you.body[0].y +1: #↖️
      left  -=5
      down  -=5
    if snake.head.x == game_state.you.body[0].x -2 and snake.head.y == game_state.you.body[0].y: #2left
      left  -=5
    if snake.head.x == game_state.you.body[0].x +2 and snake.head.y == game_state.you.body[0].y: #2right
      right -=5
    if snake.head.y == game_state.you.body[0].y -2 and snake.head.x == game_state.you.body[0].x: #2up
      up    -=5
    if snake.head.y == game_state.you.body[0].y +2 and snake.head.x == game_state.you.body[0].x: #2down
      down  -=5
